...
import metax.gui.unit_specific_settings_dialog as unit_specific_settings_dialog
import pandas as pd
import pytest
...
from metax.gui.unit_specific_settings_dialog import (
    ...
    UnitSpecificValidationResultDialog,
    validate_unit_specific_manifest_for_gui,
)
...
def test_unit_specific_gui_config_defaults_and_override():
    default = UnitSpecificGuiConfig()
    ...
    config = UnitSpecificGuiConfig(
        manifest_path="unit_specific_manifest.json",
    ...
    assert config.manifest_path == "unit_specific_manifest.json"
...
def test_unit_specific_gui_validation_missing_manifest_points_to_main_window():
    result = validate_unit_specific_manifest_for_gui(manifest_path="")

    assert result.ok is False
    assert "main Peptide Direct to OTF window" in result.message


def test_unit_specific_settings_dialog_uses_main_window_owned_values():
    app = QtWidgets.QApplication.instance() or QtWidgets.QApplication([])
    dialog = UnitSpecificSettingsDialog(
        input_intensity_prefix="Intensity_",
        diann_intensity_col="Precursor.Quantity",
        current_config=UnitSpecificGuiConfig(
            manifest_path="unit_specific_manifest.json",
            genome_threshold="q0.01",
        )
    )

    assert dialog.lineEdit_current_manifest_path.text() == "unit_specific_manifest.json"
...
def test_main_window_controls_own_genome_threshold():
    ...
    gui.unit_specific_gui_config = UnitSpecificGuiConfig(
        manifest_path="old.json",
        genome_threshold="q0.05",
        on_missing_sample="warn-skip",
    )
    gui.lineEdit_pep_direct_to_otf_unit_specific_manifest_path = QtWidgets.QLineEdit()
    gui.lineEdit_pep_direct_to_otf_unit_specific_manifest_path.setText("current.json")
    gui.comboBox_pep_direct_to_otf_use_unit_specific_genome_threshold = QtWidgets.QComboBox()
    gui.comboBox_pep_direct_to_otf_use_unit_specific_genome_threshold.addItems(["q0.05", "q0.01"])
    gui.comboBox_pep_direct_to_otf_use_unit_specific_genome_threshold.setCurrentText("q0.01")

    config = gui._get_unit_specific_gui_config_from_controls()
...
def test_unit_specific_gui_validation_passes_with_matching_peptide_header(tmp_path):
...
def test_unit_specific_gui_validation_fails_for_missing_sample_by_default(tmp_path):
    manifest_path = _write_manifest(tmp_path)
...
def test_unit_specific_gui_validation_supports_long_format_parquet(
    tmp_path,
    monkeypatch,
    intensity_col,
):
...
def test_unit_specific_gui_validation_reports_selected_diann_intensity(
    tmp_path,
    monkeypatch,
):
    manifest_path = _write_manifest(tmp_path)
...
def test_validation_result_dialog_is_scrollable_and_screen_bounded():
    app = QtWidgets.QApplication.instance() or QtWidgets.QApplication([])
    result = unit_specific_settings_dialog.UnitSpecificManifestValidationResult(
        ok=False,
    ...
    dialog = UnitSpecificValidationResultDialog(result)
...
def test_unit_specific_gui_validation_warn_skip_passes_with_missing_sample(tmp_path):
    manifest_path = _write_manifest(tmp_path)
    peptide_path = _write_peptide_table(tmp_path, ["Sequence", "Intensity_s1", "Intensity_s2"])
...
def test_unit_specific_gui_validation_fails_for_missing_peptide_column(tmp_path):
    manifest_path = _write_manifest(tmp_path)
...
def test_unit_specific_gui_validation_marks_parquet_header_skip_as_not_checked(tmp_path, monkeypatch):
    manifest_path = _write_manifest(tmp_path)
    peptide_path = tmp_path / "peptides.parquet"
